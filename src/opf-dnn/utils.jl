...
function parse_commandline()
    ...
    @add_arg_table s begin
        ...
        "--dur"
            ...
            default = 0.001
        "--traindata-size"
            help = "max traindata size"
            default = 100000
        "--kfold"
            help = "id of kfold iteration"
            arg_type = Int
            default = 1
        "--path"
...
function get_file_name(config, type, prefix=nothing)
    ...
    lr = args["lr"]
    println(args["use-constraints"])
    if args["use-constraints"]
        _PATH = config["path"] * "data/predictionsC/" * config["netname"] * "/$nepochs" * "/"
    else _PATH = config["path"] * "data/predictionsnoC/" * config["netname"] * "/$nepochs" * "/"
    end
    _nettype = args["nettype"] * "-$kfold"
    ...
    return _PATH * _nettype * _fname * _prefix * _suffix * "-$batchsize" * "-$lr" * _ext
end
