...
function dt_train(x_train, y_train)
   ...
   if type == 1
      ...
      #regressor.fit(sum(x_train,dims=2), y_train)
   ...
   elseif type == 3
      ...
      #regressor = multi.MultiOutputRegressor(regr).fit(sum(x_train,dims=2), y_train)
...
function dt_solve(x_test, regressor)
   ...
   #y_pred = regressor.predict(sum(x_test,dims=2))
...
for i in 1:5
   ...
   train, test, val = DataLoader(Sd, Flows, vm, va, pg, qg, pd,
                                 shuffle=false, batchsize=args["batchsize"], kfold_it=i)
   #
   L = Dict()
   ...
   for (j, (sd_t, flow_t, vm_t, va_t, pg_t, qg_t)) in enumerate(train)
      ...
      if type==1
         push!(max_depth, regressor_pg.tree_.max_depth)
         push!(max_depth, regressor_vm.tree_.max_depth)
         push!(max_depth, regressor_va.tree_.max_depth)
      end

      if type==2
         depth = Any[]
         for estimator in regressor_pg.estimators_
            push!(depth, estimator.tree_.max_depth)
         end
         push!(max_depth, mean(depth))

         depth = Any[]
         for estimator in regressor_vm.estimators_
            push!(depth, estimator.tree_.max_depth)
         end
         push!(max_depth, mean(depth))

         depth = Any[]
         for estimator in regressor_va.estimators_
            push!(depth, estimator.tree_.max_depth)
         end
         push!(max_depth, mean(depth))
      end
   ...
   for (j, (sd_test, flow_test, vm_test, va_test, pg_test, qg_test, pd_test)) in enumerate(test)
      ...
      #Calculate load sum
      pdm = Float64[]
      for k in 1:(length(pd_test[:,1]))

         push!(pdm, sum(pd_test[k,:]))
...
data_d = Dict(
   ...
   "loadflow" => results, "max_depth" => max_depth
)



if type == 1
   file = "data/predictions/" * name_train * "/dt-results-noshuffle.pkl"
elseif type == 2
   file = "data/predictions/" * name_train * "/rf-results-noshuffle.pkl"
elseif type == 3
   file = "data/predictions/" * name_train * "/xg-results-noshuffle.pkl"
end
