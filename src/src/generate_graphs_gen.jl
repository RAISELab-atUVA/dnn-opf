...
for h in ["1","2","3","4","5"]
    ...
    #println(length(keys(dic_pred)))
    #println(keys(dic_pred[11012]))
    err_pg = Array[]
    err_vm = Float64[]
    err_va = Float64[]
    once = true
...
file_dt = "data/predictions/" * name_train * "/dt-results-load.pkl"
...
filename3 = "data/predictions/" * name_train * "/rf-results-load.pkl"
...
save_barplot([mean(mean(mean_err_pg_dnn)), mean(mean(mean_err_pg_dwo)), mean(mean_err_pgt), mean(mean_err_pgrf)],
    ["DNN w Const","DNN w/o Const","Decision Tree", "Random Forest"],
    "Pg_errors, with mean 5 folds, $name_train, $batch, $lr, $nepoc")

save_barplot([mean(mean_err_vm_dnn), mean(mean_err_vm_dwo), mean_err_vmt, mean_err_vmrf],
 ["DNN w Const","DNN w/o Const","Decision Tree", "Random Forest"],
  "Vm_errors, with mean 5 folds, $name_train, $batch, $lr, $nepoc")

save_barplot([mean(mean_err_va_dnn), mean(mean_err_va_dwo), mean_err_vat, mean_err_varf],
 ["DNN w Const","DNN w/o Const","Decision Tree", "Random Forest"],
  "Va_errors, with mean 5 folds, $name_train, $batch, $lr, $nepoc")

save_barplot([mean(ohm_C), mean(ohm_noC), ohm_dt,ohm_rf],
    ["DNN w Const","DNN w/o Const","Decision Tree", "Random Forest"],
   "Ohm Constraint Violation, $name_train, $batch, $lr, $nepoc")

save_barplot([mean(klc_C), mean(klc_noC), klc_dt,klc_rf],
   ["DNN w Const","DNN w/o Const","Decision Tree", "Random Forest"],
  "Klc Constraint Violation, $name_train, $batch, $lr, $nepoc")
...
save_boxplot([pgallw1,pgallwo1,ferr_pgt,ferr_pgrf],
    ["DNN w Const","DNN w/o Const","Decision Tree","Random Forest"],
    "Pg_errors, $name_train, $batch, $lr, $nepoc")

save_boxplot([vmallw1,vmallwo1,ferr_vmt,ferr_vmrf],
    ["DNN w Const","DNN w/o Const","Decision Tree","Random Forest"],
    "Vm_errors, $name_train, $batch, $lr, $nepoc")

save_boxplot([vaallw1,vaallwo1,ferr_vat,ferr_varf],
    ["DNN w Const","DNN w/o Const","Decision Tree","Random Forest"],
    "Va_errors, $name_train, $batch, $lr, $nepoc")
#=
