...
skl_m = pyimport("sklearn.model_selection")
metri = pyimport("sklearn.metrics")

#------FUNC--------------------------------------------------------------------------------------------

#Fitting Decision Tree Regression to the dataset
function dt_train(x_train, y_train)
   #x_train, x_test, y_train, y_test = skl_m.train_test_split(pd, pg, test_size = 1 - args["split"])
   
   #kf =skl_m.StratifiedKFold(n_splits=5, shuffle=true, random_state=42)
   #kf.get_n_splitcv=kf(data)
...
function dt_solve(x_test, regressor)
   ...
   results = skl_t.cross_val_score(estimator= regressor, X=_X, y=_y, cv=5,
...
println(length(folds_pg[1][2])÷n_gen)

args["batchsize"] = length(folds_pg[1][2])÷n_gen
datainfo = DataInfo(data, args)
...
for i in 1:5
   ...
   regressor_pg = dt_train(folds_pd[i][1], folds_pg[i][1])
   pg_pred = dt_solve(folds_pd[i][2], regressor_pg)
   pg_pred_train = dt_solve(folds_pd[i][1], regressor_pg)
   println(length(folds_pg[i][1]))
   dic_pg["pg$i"] = folds_pg[i][2]
   dic_pg["pred_pg$i"] = pg_pred
   
   dic_pg_train["pg$i"] = folds_pg[i][1]
   dic_pg_train["pred_pg$i"] = pg_pred_train

   regressor_vm = dt_train(folds_pd[i][1], folds_vm[i][1])
   vm_pred = dt_solve(folds_pd[i][2], regressor_vm)

   dic_vm["vm$i"] = folds_vm[i][2]
   dic_vm["pred_vm$i"] = vm_pred

   regressor_va = dt_train(folds_pd[i][1], folds_va[i][1])
   va_pred = dt_solve(folds_pd[i][2], regressor_va)

   dic_va["va$i"] = folds_va[i][2]
   dic_va["pred_va$i"] = va_pred

   (sdt, flowt, vmt, vat, pgt, qgt, vm_predt, va_predt, pg_predt, pdt) = 
      Ten(folds_sd[i][2]).to(device), Ten(folds_flow[i][2]).to(device),
      Ten(vm).to(device), Ten(va).to(device), 
      Ten(pg).to(device), Ten(qg).to(device), 
      Ten(vm_pred).to(device), Ten(va_pred).to(device), Ten(pg_pred).to(device),
      Ten(folds_pd[i][2])
   ...
   L["klc"] = get_losses(constr.pbc, pdt, pg_predt, vm_predt, pf_pred, pt_pred)
...
data_d = Dict(
   ...
   "vm" => vm, "err_vm" => err_vm, "mean_err_vm" => mean(mean_err_vm),
   "va" => va, "err_va" => err_va, "mean_err_va" => mean(mean_err_va),
...
#
println(length(data_d["pg"]["pred_pg1"][:,1]))
println(length(data_d["pg"]["pred_pg1"][:,2]))
println(length(data_d["pg"]["pred_pg1"][1,:]))
println(length(data_d["pg"]["pred_pg1"][2,:]))
file = "data/predictions/" * name_train * "/dt-test.pkl"
