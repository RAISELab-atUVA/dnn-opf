...
function calculate_mse_vm(d_pg, k)
   ...
   err_pg_t = Float64[]
   mean_err_pg = Float64[]
   for j in 1:length(d_pg["vm1"])#pg1
...
function calculate_mse_va(d_pg, k)
   ...
   err_pg_t = Float64[]
   mean_err_pg = Float64[]
   for j in 1:length(d_pg["va1"])#pg1
...
args["batchsize"] = length(length(vm))
datainfo = DataInfo(data, args)
...
data_d = Dict(
   "pd" => pd, "pdm" =>pdm, 
   "pg" => pg, "err_pg" => err_pg, "mean_err_pg" => mean(mean_err_pg),
...
file = "data/predictions/" * name_train * "/xg-results-load.pkl"
